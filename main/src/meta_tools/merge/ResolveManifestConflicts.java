...
public class ResolveManifestConflicts extends QbtCommand<ResolveManifestConflicts.Options> {
    ...
    public int run(final OptionsResults<? extends Options> options) throws Exception {
        ...
        final QbtManifestVersions.LegacyParse<?, ?> lhsLegacyResult = QbtManifestVersions.parseLegacy(inLines.getLeft());
        final QbtManifestVersions.LegacyParse<?, ?> mhsLegacyResult = QbtManifestVersions.parseLegacy(inLines.getMiddle());
        final QbtManifestVersions.LegacyParse<?, ?> rhsLegacyResult = QbtManifestVersions.parseLegacy(inLines.getRight());
        ...
        return new Object() {
            public <M extends LegacyQbtManifest<M, B>, B extends LegacyQbtManifestBuilder<M, B>> int run(QbtManifestVersion<M, B> targetVersion) {
                QbtManifest lhsCurrent = lhsLegacyResult.manifest.current();
                QbtManifest mhsCurrent = mhsLegacyResult.manifest.current();
                QbtManifest rhsCurrent = rhsLegacyResult.manifest.current();
                ...
                B lhsNew = targetVersion.upgrade(lhsLegacyResult.manifest).builder();
                B mhsNew = targetVersion.upgrade(mhsLegacyResult.manifest).builder();
                B rhsNew = targetVersion.upgrade(rhsLegacyResult.manifest).builder();
                for(Step s : steps) {
                    StepResult sr = s.run();
                    for(Pair<RepoTip, VcsVersionDigest> update : sr.updates) {
                        ...
                        lhsNew = lhsNew.withRepoVersion(repo, version);
                        mhsNew = mhsNew.withRepoVersion(repo, version);
                        rhsNew = rhsNew.withRepoVersion(repo, version);
                    }
                    QbtUtils.writeLines(manifestPath, targetVersion.parser().deparse("LHS", lhsNew.build(), "MHS", mhsNew.build(), "RHS", rhsNew.build()).getRight());
                    if(sr.allBailNow) {
