...
public final class Commit extends QbtCommand<Commit.Options> {
    ...
    public int run(OptionsResults<? extends Options> options) throws Exception {
        ...
        return new Object() {
            public <M extends LegacyQbtManifest<M, B>, B extends LegacyQbtManifestBuilder<M, B>> int run(LegacyQbtManifest<M, B> manifestLegacy) {
                B newManifest = manifestLegacy.builder();
                for(Map.Entry<RepoTip, CommitMaker> e : commitsBuilder.build().entrySet()) {
                    RepoTip repo = e.getKey();
                    VcsVersionDigest repoVersion = e.getValue().commit(messageFinal);
                    newManifest = newManifest.withRepoVersion(repo, repoVersion);
                }
                manifestResult.deparse(newManifest.build());
                VcsVersionDigest commit = metaRepository.commit(amend, messageFinal, CommitLevel.MODIFIED);
